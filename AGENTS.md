...
3. **数据契约**：OpenVidHD `description.json` 索引；CPU 只产出 clip 元数据。TorchCodec 在训练 GPU 上解码（视频默认 NVDEC；序列目录内 `sequence.mp4` sidecar 优先于逐帧图像）。分辨率与 deploy 对齐（LR 360×640 canvas，HR 1080×1920）。解码与几何变换以 `data/decode.py`、`data/openvid.py`、`data/mlvc_loader.py` 为准。
