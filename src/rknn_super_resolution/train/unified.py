...
from rknn_super_resolution.distributed import (
    distributed_session,
    unwrap_model,
    wrap_training_model,
)
...
def validate_training_args(args: argparse.Namespace) -> None:
    ...
    for phase, safety_steps, min_evaluations in phase_guards:
        if safety_steps < args.val_every * min_evaluations:
            raise ValueError(
                f"{phase}_safety_max_steps must allow at least {min_evaluations} validations"
            )
...
def _stop_reason(state: EarlyStopState) -> str:
    plateau = (
        state.evaluations >= state.min_evaluations and state.patience_counter >= state.patience
    )
        ...
        try:
            ...
            if resume_raw is not None and resume_phase != FLOAT:
                resume_early_stop = _early_stop(
                    args.observer_patience if resume_phase == QAT_OBSERVE else args.qat_patience,
                    args.observer_min_delta if resume_phase == QAT_OBSERVE else args.qat_min_delta,
