...
from rknn_super_resolution.utils.traceml_profiling import trace_training_step
...
class StepTrainer:
    ...
    def _log_plan(self) -> None:
        if not self.ctx.is_main:
            return
        plan = "plan: safety_max_steps={} val_every={} save_every={} log_every={}" + (
    ...
    def run(self) -> int:
        ...
        if self.ctx.is_main and self.val_loader is not None and self.validation_config.data_preview:
            try:
                run_training_data_preview(
                    self.val_loader,
