...
from rk3588_mobile_sr.data.div2k_loader import collect_paired_paths
...
def unpack_patch_pair(data: bytes, meta: LMDBMeta) -> tuple[torch.Tensor, torch.Tensor]:
    ...
    hr_arr = (
        np.frombuffer(data[meta.lr_bytes : meta.record_bytes], dtype=np.uint8)
        .reshape(meta.hr_shape)
        .copy()
    )
...
def build_div2k_lmdb(
    *,
    ...
    if (output_dir / "data.mdb").exists():
        raise FileExistsError(f"LMDB already exists at {output_dir}. Delete it before rebuilding.")
...
class DIV2KLMDBDataset(Dataset):
    ...
    def _ensure_open(self) -> tuple[lmdb.Environment, LMDBMeta]:
        if self._env is None:
            ...
            if self.augment and self._meta.offline_augment and not self._offline_augment_warned:
                import warnings
