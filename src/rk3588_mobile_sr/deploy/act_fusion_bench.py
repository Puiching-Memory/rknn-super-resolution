...
from rk3588_mobile_sr.deploy.rknn_env import resolve_rknn_python
from rk3588_mobile_sr.deploy.rknn_eval import _rknn_output_to_hwc, psnr_numpy
...
def _benchmark_one_rknn(
    ...
    *,
    ...
    try:
        ...
        if build_ok:
            ...
            for rgb in images:
                ...
                rknn_out = _rknn_output_to_hwc(rknn.inference(inputs=[batch], data_format="nhwc")[0])
    ...
    fused_candidates = [
        op
        for op in npu_ops
        if op.startswith("Conv") and op not in {"Conv", "ConvTranspose"}
    ]
    ...
    conv_act_fused = fused_npu_op is not None and not any(op in cpu_ops for op in activation_cpu_ops)
...
def _benchmark_one_subprocess(
    ...
    *,
    ...
    cmd = [
        sys.executable,
        "-m",
        "rk3588_mobile_sr.deploy.act_fusion_bench",
        "--phase",
        "single",
        "--activations",
        name,
    ...
    if proc.returncode != 0:
        ...
        raise SystemExit(f"RKNN subprocess failed for {name} (rc={proc.returncode}): {' | '.join(tail)}")
...
def _run_rknn_phase(args: argparse.Namespace) -> list[ActBenchRow]:
    ...
    for name, onnx_path in onnx_paths.items():
        ...
        if use_subprocess:
            rows.append(_benchmark_one_subprocess(name, onnx_path, args, calib=calib, log_dir=log_dir))
        else:
            rows.append(
                _benchmark_one_rknn(
                    name,
